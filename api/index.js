  ...
  const path =
    col === "policyReminders"
      ...
      : col === "partner_applications"
        ? "partners"
        : col;
  app.get(`/api/${path}`, async (c) => c.json(toArray(await dbGet(col))));
...
// Revenue Special Routes
app.get("/api/revenue/cibil", async (c) =>
  c.json((await dbGet("cibil_requests")) || {}),
);
app.get("/api/revenue/manual", async (c) =>
  c.json((await dbGet("manual_revenue")) || {}),
);
app.post("/api/revenue/manual", async (c) =>
  c.json({ id: await dbPush("manual_revenue", await c.req.json()) }, 201),
);
app.put("/api/revenue/manual/:id", async (c) => {
  await dbUpdate(`manual_revenue/${c.req.param("id")}`, await c.req.json());
  return c.json({ ok: true });
});
app.get("/api/revenue/cashfree", async (c) =>
  c.json((await dbGet("cashfree_revenue")) || {}),
);
app.put("/api/revenue/cashfree/:id", async (c) => {
  await dbUpdate(`cashfree_revenue/${c.req.param("id")}`, await c.req.json());
  return c.json({ ok: true });
});
