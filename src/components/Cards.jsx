...
export const Cards = () => {
  ...
  const getVisibleServices = () => {
    if (!isMobileOrTablet) {
      return services; // Show all on desktop
    }
      ...
      {!isMobileOrTablet && (
        ...
        >
          {services.map((service, index) => (
            <div
              key={index}
              className={`service-card spec ${service.className || ""}`}
              style={{ justifyContent: "start", gap: ".75em" }}
              onClick={() => handleCardClick(service.url)}
            >
              <div className="service-icon">{service.media}</div>
              <div className="service-title">{service.title}</div>
              <div className="service-overview">{service.description}</div>
            </div>
          ))}
        </div>
      )}
            ...
            >
              {getVisibleServices().map((service, idx) => (
                <motion.div
                  ...
                  onClick={() => handleCardClick(service.url)}
                  initial={
                    slideDirection === "left" ? "hiddenLeft" : "hiddenRight"
                  }
                  animate="visible"
