...
export const AdminTable = () => {
  ...
  const downloadCsv = () => {
    ...
    const headers = ['id', 'firstName', 'lastName', 'email', 'phone', 'reason', 'createdAt']
    ...
    a.href = url
    a.download = `consultations_${new Date().toISOString().slice(0,19).replace(/[:T]/g,'-')}.csv`
  ...
  return (
    <div className="admin-table">
      ...
      <div className="admin-actions">
        <button className="btn" onClick={downloadCsv} disabled={rows.length===0}>Download CSV</button>
      </div>
